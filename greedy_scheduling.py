...
def find_greedy_action(eval_env): ## maximal age difference
    ...
    greedy_age_BS = {x:eval_env.BS_age[x] for x in eval_env.user_list}
    if verbose:
        print(f"age of users at BS is {greedy_age_BS} where age at BS is {eval_env.BS_age}, age at UAV is {eval_env.UAV_age}")
        # time.sleep(5)
   
        print(f"greedy_age_BS = {greedy_age_BS}")
   
    ## UAV selection begins

    sampleDict_copy = copy.deepcopy(greedy_age_BS)
 ...
 ############################################
    ...
    greedy_action = list(itertools.product([updated_users], [selected_users_overall]))
        
    ## finally collect the result and compare with the action space of the environment
    updated_users = sorted(greedy_action[0][0])
    sampled_users= sorted(greedy_action[0][1])
...
def greedy_scheduling(I, drones_coverage, folder_name, deployment, packet_update_loss, packet_sample_loss, periodicity):  ## maximal age difference
    print(f"\ngreedy started for {I} users , coverage = {drones_coverage} with update_loss = {packet_update_loss}, sample_loss = {packet_sample_loss}, periodicity = {periodicity} and {deployment} deployment")
    print(f"\ngreedy started for {I} users , coverage = {drones_coverage} with update_loss = {packet_update_loss}, sample_loss = {packet_sample_loss}, periodicity = {periodicity} and {deployment} deployment", file = open(folder_name + "/results.txt", "a"), flush = True)
    ...
    for ep in range(random_episodes): # how many times the random policy will be run, similar to episode
        ...
        if ep==0:
            # time.sleep(10)
            # pass
            print(f"\ngreedy scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, BS_capacity is {eval_env.BS_capacity}, UAV_capacity = {eval_env.UAV_capacity}, action space size is {eval_env.action_size} and they are {eval_env.actions_space} \n\n", file = open(folder_name + "/action_space.txt", "a"), flush = True)
            print(f"\ngreedy scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, BS_capacity is {eval_env.BS_capacity}, UAV_capacity = {eval_env.UAV_capacity}, action space size is {eval_env.action_size} ", file = open(folder_name + "/results.txt", "a"), flush = True)
    ...
    pickle.dump(dd_age_dist_UAV, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_age_dist_UAV.pickle", "wb"))
    pickle.dump(dd_age_dist_BS, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_age_dist_BS.pickle", "wb"))

    pickle.dump(eval_env.sample_time, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_sample_time.pickle", "wb"))    
