...
# comet = True
CSI_as_state = False
sample_error_in_CSI = True ## has no meaning if CSI_as_state=False; if CSI_as_state True, this True will mean only UAV CSI is included in state
...
fc_layer_params = (1024, 2048, 1024) # for 5 user 2 SC (32,16) # for 3 user 1 SC
...
class UAV_network(py_environment.PyEnvironment):   # network of UAVs not just a single one
    ...
    def __init__(self, n_users, coverage, name, folder_name, packet_update_loss, packet_sample_loss, periodicity): # number of total user and the user-coverage for every UAV
        ...
        param periodicity    : dict, period of traffic generation by ground users
        ...
        if CSI_as_state:
            if sample_error_in_CSI:
                self._observation_spec = array_spec.BoundedArraySpec(shape=( 1 + 3*n_users + len(self.UAV_list)), dtype=np.float32, minimum=0, maximum=MAX_STEPS+2, name='current_state') # + 1 for the current time
                ...
                # first one is the current time
            else:
                self._observation_spec = array_spec.BoundedArraySpec(shape=(1 + 2*n_users + len(self.UAV_list) + 1), dtype=np.float32, minimum=0, maximum=MAX_STEPS+2, name='current_state') # + 1 for the current time
                self._state = np.concatenate((list(self.current_step), [1]*2*(n_users), list(self.update_loss.values())), axis=None) #
            
        else:
            self._observation_spec = array_spec.BoundedArraySpec(shape=(1 + 2*n_users), dtype=np.float32, minimum=0, maximum=MAX_STEPS+2, name='current_state') # + 1 for the current time
    ...
    def _reset(self):
        ...
        if CSI_as_state: # csi as state needed
            ...
            self._state = np.concatenate((list(self.current_step), list(self.current_step), [1]*2*(len(self.user_list))), axis=None)
        ...
        # step 1 so BS has nothing to get from UAV
            ...
            for i in self.user_list: ## updating
                if i in updated_users:
                    ...
                    if verbose:
                        print(f" for user {i}, chance_update_loss = {chance_update_loss} and self.update_loss = {self.update_loss[i]} ")
